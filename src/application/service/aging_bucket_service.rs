//! Application service for AgingBucket entities
//!
//! Generated by metaphor-schema. Do not edit manually.
//!
//! Type alias over `GenericCrudService` — all CRUD logic lives in backbone-core.
//! For custom behaviour, create `aging_bucket_service_custom.rs` (preserved on regen).

use backbone_core::GenericCrudService;

use crate::presentation::dto::{CreateAgingBucketDto, UpdateAgingBucketDto};
use crate::domain::entity::AgingBucket;
use crate::infrastructure::persistence::AgingBucketRepository;

/// Application service for AgingBucket entities.
///
/// All 12 standard CRUD operations (list, create, get, update, patch,
/// soft_delete, restore, empty_trash, bulk_create, upsert, find_by_id,
/// list_deleted) are provided by `GenericCrudService`.
///
/// Custom extensions go in `aging_bucket_service_custom.rs`:
///
/// ```rust,ignore
/// pub struct AgingBucketServiceCustom {
///     inner: Arc<AgingBucketService>,
///     // add domain-specific dependencies here
/// }
/// ```
pub type AgingBucketService = GenericCrudService<
    AgingBucket,
    CreateAgingBucketDto,
    UpdateAgingBucketDto,
    AgingBucketRepository,
>;

// <<< CUSTOM
// END CUSTOM
