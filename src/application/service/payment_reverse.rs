...
impl PaymentWriteService {
    ...
    pub async fn build_reversal_post(
        ...
        let env = AccountingPostEnvelope {
            idempotency_key: format!("reversal:{payment_id}"),
    ...
    pub async fn reverse_payment(
        ...
        // committed) reverses; `cancelled` re-reverses idempotently. A draft never posted, and a
        // rejected payment never will — neither has GL to unwind.
        if status != "in_flight" && status != "paid" && status != "cancelled" {
            return Err(PaymentError::NotReversible(status));
        }
        // `posting_state` is the GL-sync truth the cancel CAS below keys on: refusing a
        // non-`posted` entry HERE turns a would-be reversal-journal-without-emission into a clean
        // error, instead of relying on `mark_cancelled` matching zero rows after the sink ran.
        ...
        match sink.post(&env).await {
            ...
            Err(rej) => Err(PaymentError::GlRejected {
                code: rej.code,
                message: rej.message,
            }),
        }
    ...
    async fn emit_cancelled(
        ...
        let allocations: Vec<SettledInvoice> = alloc_rows
            ...
            .map(|r| SettledInvoice {
                ...
                discount_amount: (r.discount_amount > Decimal::ZERO).then_some(r.discount_amount),
            })
            .collect();
        self.sink
            .publish(PaymentEvent::PaymentCancelled(PaymentCancelled {
                payment_id,
