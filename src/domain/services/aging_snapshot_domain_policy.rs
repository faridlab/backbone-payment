//! Domain policy for AgingSnapshot aggregate
//!
...
/// Domain policy for AgingSnapshot — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<AgingSnapshot>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type AgingSnapshotDomainPolicy = PermitAllPolicy<AgingSnapshot>;
