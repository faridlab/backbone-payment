//! Domain policy for PaymentEntry aggregate
//!
...
/// Domain policy for PaymentEntry — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<PaymentEntry>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type PaymentEntryDomainPolicy = PermitAllPolicy<PaymentEntry>;
