...
async fn receive_math_and_post() {
    ...
    let (company, bank, ar, customer, inv) = (
        Uuid::new_v4(),
        Uuid::new_v4(),
        Uuid::new_v4(),
        Uuid::new_v4(),
...
async fn pay_supplier_post() {
    ...
    let p = NewPayment {
        ...
        reference_no: None,
        allocations: vec![alloc(inv, "purchase", "500000")],
...
async fn posting_is_idempotent() {
    ...
    assert_eq!(
        settled, 1,
        "PaymentSettled emitted exactly once across two posts"
    );
}
...
async fn validation_gates() {
    ...
    let e = w
        .create_payment(receive(company, bank, ar, customer, "0", vec![]))
        .await
        .unwrap_err();
    ...
    let num = uq("DUP");
    p.payment_number = num.clone();
    w.create_payment(p.clone()).await.unwrap();
    p.payment_number = num;
    assert!(matches!(
        w.create_payment(p).await.unwrap_err(),
        PaymentError::DuplicateNumber(_)
    ));
}
...
async fn fully_allocated_no_on_account() {
    ...
    let id = w
        .create_payment(receive(
            ...
            customer,
            "250000",
            vec![alloc(inv, "sales", "250000")],
        ))
        .await
        .unwrap();
    let un: Decimal =
        sqlx::query_scalar("SELECT unallocated_amount FROM payment.payment_entries WHERE id=$1")
            .bind(id)
            .fetch_one(&pool)
            .await
            .unwrap();
    ...
    assert!(
        !rec.events
            ...
            .unwrap()
            .iter()
            .any(|e| matches!(e, PaymentEvent::PaymentReceivedOnAccount(_))),
        "no remainder → no on-account event"
    );
}
