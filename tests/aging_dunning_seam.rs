...
struct BillingReceivablesAdapter {
    pool: PgPool,
}
#[async_trait::async_trait]
...
async fn aging_buckets_and_dunning_escalate_across_seam() {
    ...
    let inv = billing
        .create_sales_invoice(NewSalesInvoice {
            ...
            source_so_id: None,
            posting_date: due_45d_ago,
            due_date: Some(due_45d_ago),
    ...
    let bucket_31_60: Decimal =
        sqlx::query_scalar("SELECT bucket_31_60 FROM payment.aging_snapshots WHERE id = $1")
            .bind(snapshot_id)
            .fetch_one(&pool)
            .await
            .unwrap();
