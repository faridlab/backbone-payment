...
async fn seed_coa(pool: &PgPool) -> (Uuid, HashMap<&'static str, Uuid>) {
    ...
    let coa: &[(&str, &str, &str, &str, &str, bool)] = &[
        ...
        ("4000", "Pendapatan", "revenue", "operating_revenue", "credit", false),
...
async fn settlement_across_three_modules() {
    ...
    assert_eq!(residual(&pool, company, "order", inv, coa["1200"]).await, d("1000000.00"));
    ...
    assert_eq!(residual(&pool, company, "order", inv, coa["1200"]).await, d("400000.00"));
    assert_eq!(residual(&pool, company, "payment", pay_a, coa["1200"]).await, Decimal::ZERO);
    assert_eq!(full_groups(&pool, company).await, 0);
    // The seam's invariant: outstanding == grand_total − Σ settlement edges.
    assert_eq!(d("400000.00"), d("1000000.00") - settlement_edges(&pool, company).await.0);
    ...
    assert_eq!((sum, n), (d("1000000.00"), 2), "one settlement edge per payment");
    assert_eq!(residual(&pool, company, "order", inv, coa["1200"]).await, Decimal::ZERO);
    assert_eq!(residual(&pool, company, "payment", pay_a, coa["1200"]).await, Decimal::ZERO);
    assert_eq!(residual(&pool, company, "payment", pay_b, coa["1200"]).await, Decimal::ZERO);
...
async fn apply_settlements(billing: &BillingWriteService, sink: &AccountingReconcileSink, rec: &RecordingPaySink, payment_id: Uuid) -> Decimal {
    ...
    for a in &settled.allocations {
        let applied = billing.apply_settlement(settled.company_id, a.invoice_ref, &a.invoice_kind, a.amount, payment_id, sink).await.unwrap().applied;
...
async fn reverse_settlements(billing: &BillingWriteService, sink: &AccountingReconcileSink, rec: &RecordingPaySink, payment_id: Uuid) {
    let evts = rec.events.lock().unwrap().clone();
    ...
    for a in &cancelled.allocations {
        billing.reverse_settlement(cancelled.company_id, a.invoice_ref, &a.invoice_kind, a.amount, payment_id, sink).await.unwrap();
    }
}
...
async fn reverse_payment_restores_invoice_and_is_idempotent() {
    ...
    assert_eq!(full_groups(&pool, company).await, 1);
    assert!(line_reconciled(&pool, company, "order", inv, coa["1200"]).await.0);
    ...
    assert_eq!(settlement_edges(&pool, company).await, (Decimal::ZERO, 0), "the settlement edge is unlinked");
    assert_eq!(residual(&pool, company, "order", inv, coa["1200"]).await, d("1000000.00"), "invoice A/R reopened");
    let (inv_rec, inv_full) = line_reconciled(&pool, company, "order", inv, coa["1200"]).await;
    ...
    assert!(pay_rec, "payment original credit paired with the reversal debit");
    assert!(pay_full.is_some(), "the reverse-then-reconcile pair forms its own group");
...
async fn racing_payments_reconcile_via_clamp_and_on_account() {
    ...
    for _i in 0..2 {
        ...
        let a = billing.apply_settlement(company, inv, "sales", d("600000"), pay, &sink).await.unwrap().applied;
        if _i == 1 { applied_second = a; }
        pays.push(pay);
    }
    ...
    let (sum, n) = settlement_edges(&pool, company).await;
    assert_eq!((sum, n), (d("1000000.00"), 2), "edges clamp together with the subledger");
    assert_eq!(residual(&pool, company, "order", inv, coa["1200"]).await, Decimal::ZERO);
    assert_eq!(residual(&pool, company, "payment", pay1, coa["1200"]).await, Decimal::ZERO);
    assert_eq!(residual(&pool, company, "payment", pay2, coa["1200"]).await, d("200000.00"),
...
async fn two_allocations_write_two_edges_and_reverse_unwinds_both() {
    ...
    let inv_b = billing.create_sales_invoice(new_invoice(d("400000"))).await.unwrap();
    ...
    let pay = payment.create_payment(NewPayment {
        payment_number: uq("PE"), company_id: company, branch_id: None, payment_type: "receive".into(),
        party_type: Some("customer".into()), party_id: Some(customer), posting_date: day(), currency: None,
        mode_of_payment_id: None, bank_account_id: coa["1110"], party_account_id: coa["1200"], paid_amount: d("900000"),
        reference_no: None, allocations: vec![
            NewAllocation { invoice_ref: inv_a, invoice_kind: "sales".into(), amount: d("500000") },
            NewAllocation { invoice_ref: inv_b, invoice_kind: "sales".into(), amount: d("400000") },
        ],
        withholding_amount: rust_decimal::Decimal::ZERO,
        withholding_account_id: None,
        withholding_tax_type: "none".into(),
    ...
    assert_eq!(on_account, Decimal::ZERO, "nothing on-account — both allocations absorbed");
    ...
    assert_eq!(invoice_row(&pool, inv_a).await, (d("0.00"), "paid".to_string()));
    assert_eq!(invoice_row(&pool, inv_b).await, (d("0.00"), "paid".to_string()));
    let (sum, n) = settlement_edges(&pool, company).await;
    assert_eq!((sum, n), (d("900000.00"), 2), "one edge per allocation");
    assert_eq!(residual(&pool, company, "order", inv_a, coa["1200"]).await, Decimal::ZERO);
    assert_eq!(residual(&pool, company, "order", inv_b, coa["1200"]).await, Decimal::ZERO);
    assert_eq!(residual(&pool, company, "payment", pay, coa["1200"]).await, Decimal::ZERO);
    assert_eq!(full_groups(&pool, company).await, 1, "one connected component, one group");
    ...
    payment.reverse_payment(pay, &gl).await.unwrap();
    reverse_settlements(&billing, &sink, &recorder, pay).await;
    assert_eq!(invoice_row(&pool, inv_a).await, (d("500000.00"), "submitted".to_string()), "A restored");
    assert_eq!(invoice_row(&pool, inv_b).await, (d("400000.00"), "submitted".to_string()), "B restored");
    assert_eq!(settlement_edges(&pool, company).await, (Decimal::ZERO, 0), "both edges unlinked");
    assert_eq!(residual(&pool, company, "order", inv_a, coa["1200"]).await, d("500000.00"));
    assert_eq!(residual(&pool, company, "order", inv_b, coa["1200"]).await, d("400000.00"));
    assert!(!line_reconciled(&pool, company, "order", inv_a, coa["1200"]).await.0);
    assert!(!line_reconciled(&pool, company, "order", inv_b, coa["1200"]).await.0);
    let paired: i64 = sqlx::query_scalar(
        "SELECT COUNT(*) FROM accounting.partial_reconciles WHERE company_id=$1 AND metadata->>'rule'='reverse_then_reconcile'")
        .bind(company).fetch_one(&pool).await.unwrap();
    assert_eq!(paired, 1, "the payment's own credit↔reversal pair is the only survivor");
    // Ledger nets zero for the whole round trip: Dr A/R (invoices) 900k, Cr A/R 900k, Dr 900k back.
    assert_eq!(ar_party_credit(&pool, coa["1200"], customer).await, d("-900000.00"));
}
